...
class TestFridge(unittest.TestCase):
    ...
    def test_should_set_the_current_time_to_given_string(self):
        ...
        self.assertEqual(fridge._current_time.strftime("%d/%m/%y"), '18/10/21')

    def test_should_simulate_day_over_to_next_date(self):
        fridge = Fridge()

        fridge.set_current_date('18/10/21')
        fridge.simulate_day_over()

        self.assertIsInstance(fridge._current_time, datetime.datetime)
        self.assertEqual(fridge._current_time.strftime("%d/%m/%y"), '19/10/21')
    ...
    def test_should_signal_fridge_door_closed(self):
        ...
        self.assertEqual(fridge._state, FridgeState.DOOR_CLOSED)

    def test_should_not_remove_item_when_fridge_door_is_closed(self):
        fridge = Fridge()
        fridge.set_current_date('18/10/21')
        with self.assertRaises(fridge.DoorClosed):
            fridge.scan_removed_item('jam')

    def test_should_not_remove_item_when_there_is_no_matching_item_with_name(self):
        fridge = Fridge()
        fridge.set_current_date('18/10/21')
        fridge.signal_fridge_door_opened()
        fridge.scan_added_item(name='bread', expiry='2021', condition=str(ItemCondition.SEALED))

        with self.assertRaises(fridge.CannotRemoveItem):
            fridge.scan_removed_item('jam')
