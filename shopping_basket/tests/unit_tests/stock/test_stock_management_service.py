...
class StockManagementServiceShould(TestCase):
    ...
    def test_update_stock_levels_for_items_purchased(self):
        ...
        self.stock_repository.save_stock.assert_has_calls(
            [call(stock=SOLD_STOCK_VIDEO)]
        )
        self.message_bus.handle.assert_called_once_with(
            event=StockIsLow(product_id=SOLD_STOCK_VIDEO.product_id, order_quantity=5)
        )

    def test_increase_stock_levels_for_product(self):
        self.stock_repository.find_by_id.return_value = SOLD_STOCK_VIDEO
        self.stock_management.increase_stock(product_id=PRODUCT_ID_VIDEO, quantity=5)
        self.stock_repository.save_stock.assert_called_once_with(
            stock=PURCHASED_STOCK_VIDEO
        )
