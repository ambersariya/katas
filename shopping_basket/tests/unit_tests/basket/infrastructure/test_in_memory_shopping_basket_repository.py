...
class TestInMemoryShoppingBasketRepositoryShould:
    ...
    def test_create_shopping_basket_only_when_we_add_an_item(self, date_provider,
                                                             in_memory_shopping_basket_repo) -> None:
        date_provider.current_date.return_value = "15/06/2022"
        in_memory_shopping_basket_repo.add_item(
            item=BASKET_ITEM_LORD_OF_THE_RINGS_QUANTITY_FIVE,
            user_id=USER_ID
        )
    ...
    def test_save_more_than_one_kind_of_item_with_different_id(self,
                                                               in_memory_shopping_basket_repo,
                                                               date_provider) -> None:
        date_provider.current_date.return_value = "15/06/2022"
        in_memory_shopping_basket_repo.add_item(item=BASKET_ITEM_LORD_OF_THE_RINGS_QUANTITY_FIVE,
                                                user_id=USER_ID)
        in_memory_shopping_basket_repo.add_item(item=BASKET_ITEM_BREAKING_BAD_QUANTITY_TWO,
                                                user_id=USER_ID)
