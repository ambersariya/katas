...
from constants import BASKET_ITEM_LORD_OF_THE_RINGS_QUANTITY_FIVE, BASKET_ITEM_BREAKING_BAD_QUANTITY_TWO, USER_ID
...
class InMemoryShoppingBasketRepositoryShould(TestCase):
    ...
    def test_create_shopping_basket_only_when_we_add_an_item(self):
        ...
        self.repository.add_item(item=BASKET_ITEM_LORD_OF_THE_RINGS_QUANTITY_FIVE, user_id=USER_ID)
    ...
    def test_save_more_than_one_kind_of_item_with_different_id(self):
        self.date_provider.current_date.return_value = "15/06/2022"
        self.repository.add_item(item=BASKET_ITEM_LORD_OF_THE_RINGS_QUANTITY_FIVE, user_id=USER_ID)
        self.repository.add_item(item=BASKET_ITEM_BREAKING_BAD_QUANTITY_TWO, user_id=USER_ID)

        result = self.repository.basket_for(user_id=USER_ID)

        self.assertIsInstance(result, ShoppingBasket)
        self.assertEqual(2, len(result.items))
        self.assertEqual(BASKET_ITEM_LORD_OF_THE_RINGS_QUANTITY_FIVE, result.items[0])
        self.assertEqual(BASKET_ITEM_BREAKING_BAD_QUANTITY_TWO, result.items[1])
