...
class ManageStockAvailabilityShould(BaseTestCase):
    ...
    def test_raise_error_when_not_enough_product_in_stock(self):
        with self.assertRaises(InsufficientStockError):
            self._add_item(product_id=PRODUCT_ID_BOOK, quantity=10)

    def test_add_item_to_basket_when_stock_is_sufficient(self):
        self._add_item(product_id=PRODUCT_ID_BOOK, quantity=2)
        actual_stock = self.stock_repository.find_by_id(PRODUCT_ID_BOOK)
        expected_stock = Stock(product_id=PRODUCT_ID_BOOK, available=3, reserved=2, min_available=5)
