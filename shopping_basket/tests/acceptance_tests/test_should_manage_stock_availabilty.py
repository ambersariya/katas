...
class TestManageStockAvailabilityShould:
    def test_raise_error_when_not_enough_product_in_stock(self, shopping_basket_service):
        with pytest.raises(InsufficientStockError):
            shopping_basket_service.add_item(
                user_id=USER_ID, product_id=PRODUCT_ID_LORD_OF_THE_RINGS, quantity=10
            )

    def test_add_item_to_basket_when_stock_is_sufficient(
        self, shopping_basket_service, stock_repository
    ):
        shopping_basket_service.add_item(
            user_id=USER_ID, product_id=PRODUCT_ID_LORD_OF_THE_RINGS, quantity=2
        )
